...
> **ダブル Nginx 構成について**: このフロントエンドコンテナは `nginx:alpine` を使って静的ファイルを配信します。第16章で設定するリバースプロキシ Nginx とは別の Nginx です。構成は「リバースプロキシ Nginx → フロントエンド Nginx（静的ファイル配信）」となります。フロントエンドコンテナの Nginx はデフォルト設定で静的ファイルを返すだけなので、カスタム設定は不要です。
...
# API の Dockerfile のコンテキストがプロジェクトルートの場合
...
services:
  ...
  nginx:
    ...
    ports:
      - "80:80"
      # 注意: SSL/TLS は Cloudflare Tunnel が処理するため、
      # Nginx 側で 443 ポートや証明書の設定は不要です
    volumes:
